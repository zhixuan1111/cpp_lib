...
- [七、数据库(SQLite)](#七数据库sqlite)
  - [7.1 增（INSERT）操作](#71-增insert操作)
  - [7.2 查（SELECT）操作](#72-查select操作)
  - [7.3 改（UPDATE）操作](#73-改update操作)
  - [7.4 删（DELETE）操作](#74-删delete操作)
...
1. 首部结构（20 字节固定部分 ）
有源端口、目的端口、序号、确认号、数据偏移、保留、窗口、校验和、紧急指针等字段，序号对字节流编号，确认号实现可靠传输，窗口字段用于流量控制，标志位（如 SYN、FIN 等 ）控制连接建立 / 终止等操作 。
### 6.2.3 关键机制
流量控制（控制发送速率防溢出 ）、拥塞控制（应对网络拥塞，如慢启动、拥塞避免等算法 ），保障数据有序、可靠传输 。
